...
function handleDiscovery( request )
{
	...
	request.fhemReq( 'jsonlist2 EchoCap=scene EchoDesc',
			( result ) =>
			{
				...
				request.fhemReq( 'get ' + getScenes.substr(1) + ' scenes',
						( result ) =>
						{
							let endpoints = request._response.payload.endpoints = [];
							let scenes = result.split(/\n\n/).map( (str) => { return str.split(/\n/); } );
							for ( let i = 0; i < devices.length; ++i )
								for ( let friendlyName of scenes[i] )
									endpoints.push(
									{
										endpointId: friendlyName.replace(/[^a-zA-Z0-9]/g,'_'),
										friendlyName,
										description: friendlyName + " " + devices[i].desc,
										manufacturerName: 'FHEM scene',
										displayCategories: [ "ACTIVITY_TRIGGER" ],
										cookie: { dev: devices[i].name, scene: friendlyName },
										capabilities: [ {
										  type: "AlexaInterface",
...
function controlRequest( request )
{
	...
	// let target = request.event.directive.header.name == 'Activate' ? 'on' : 'off';
	request.fhemReq( 'set ' + dev + ' scene ' + request.event.directive.endpoint.endpointId,
			( result ) =>
			{
				request._response.header.name = 'ActivationStarted';
